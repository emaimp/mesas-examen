...
#
# Obtiene la cantidad de aprobados y desaprobados en mesas de examen de una carrera
#
def aprobacion_examenes_carrera(session: Session, carrera_id: int) -> schemas.ExamApprovalCareer:
    # Busca la carrera por su ID
    carrera = session.get(models.Carreras, carrera_id)
    # Si la carrera no existe, lanza un error
    if not carrera:
