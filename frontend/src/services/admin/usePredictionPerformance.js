...
export function usePredictionPerformance () {
   ...
   */
  const fetchPredictionPerformance = async carreraId => {
    // Verifica si se proporcionó un ID de carrera
    if (!carreraId) {
      console.warn('fetchPredictionPerformance: No se proporcionó carreraId')
      return null
    }
    try {
  ...
  return {
    // Expone la función fetchPredictionPerformance para ser usada en otros componentes
